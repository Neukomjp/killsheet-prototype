...
export async function POST(req: Request) {
    try {
        ...
        if (step === 1) {
            ...
            promptText = `以下のメモから、氏名、職種、および主要スキルを抽出してJSON形式で返してください。
...
${text}`;
            ...
            promptText = `以下の「自己PRや強みなどのメモ」から、エンジニアとしての魅力的な職務要約（サマリー）を2〜3文で抽出・作成してJSON形式で返してください。
...
${text}`;
            ...
            promptText = `以下の「プロジェクト履歴のメモ」から、プロジェクトごとの履歴情報を抽出してJSON形式で返してください。
